...
import Data.Aeson.Decoding.Conversion (lbsSpace, toResultValue, unResult)
...
instance (ConstructorNames f, SumFromString f) => GFromJSONKey f
...
instance
  ...
  gParseJSON' p =
    ( unTagged ::
        Tagged allNullary (Parser ((a :+: b) _d)) ->
        Parser ((a :+: b) _d)
    )
      . parseSum p
  {-# INLINE gParseJSON' #-}
...
instance (SumFromString a, SumFromString b) => SumFromString (a :+: b) where
  parseSumFromString opts key =
    (L1 <$> parseSumFromString opts key)
  ...
  case sumEncoding opts of
    ...
    TwoElemArray ->
      withArray tname $ \arr -> case V.length arr of
        2
          | String tag <- V.unsafeIndex arr 0 ->
...
instance
  (FromTaggedObject arity a, FromTaggedObject arity b) =>
...
instance
  ...
  ConsFromJSON' arity (S1 s a) True
  where
...
instance {-# OVERLAPPING #-} ConsFromJSON' arity U1 False where
...
instance {-# OVERLAPPING #-} (GFromJSON arity f) => ConsFromJSON' arity (S1 s f) False where
...
instance
  ( RecordFromJSON' arity a
  , RecordFromJSON' arity b
  ) =>
  RecordFromJSON' arity (a :*: b)
  where
...
instance {-# OVERLAPPABLE #-} (RecordFromJSON' arity f) => RecordFromJSON' arity (M1 i s f) where
  recordParseJSON' args obj = M1 <$> recordParseJSON' args obj
  {-# INLINE recordParseJSON' #-}

instance
  (Selector s, FromJSON a, Generic a, K1 i a ~ Rep a) =>
            ...
            contextCons cname tname $
              fail $
                "expected an Array of length "
                  ++ show lenProduct
                  ++ ", but encountered an Array of length "
                  ++ show lenArray
...
instance
  ( ProductFromJSON arity a
  , ProductFromJSON arity b
  ) =>
  ProductFromJSON arity (a :*: b)
  where
...
instance FromJSON Ordering where
  parseJSON = withText "Ordering" $ \s ->
    case s of
      ...
      _ ->
        fail $
          "parsing Ordering failed, unexpected "
            ++ show s
...
instance (FromJSONKey k, Ord k) => FromJSON1 (M.Map k) where
  ...
  liftParseJSON _ p _ = case fromJSONKey of
    ...
    FromJSONKeyValue f -> withArray "Map" $ \arr ->
      fmap M.fromList
        . Tr.sequence
        . zipWith (parseIndexedJSONPair f p) [0 ..]
        . V.toList
        $ arr
    where
...
instance FromJSON CalendarDiffTime where
  parseJSON = withObject "CalendarDiffTime" $ \obj ->
    CalendarDiffTime
      <$> obj .: "months"
      <*> obj .: "time"

instance FromJSON CalendarDiffDays where
  parseJSON = withObject "CalendarDiffDays" $ \obj ->
    CalendarDiffDays
      <$> obj .: "months"
      <*> obj .: "days"
