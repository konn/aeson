...
import Data.Time.Calendar (Day, Year, fromGregorianValid)
import Data.Time.Calendar.Month (Month, fromYearMonthValid)
...
parseTimeOfDay = parseTimeOfDay_ kontEOF $ \_ _ _ c _ -> unexpectedChar c "end-of-input"
  where
...
twoDigits ::
  ...
  Either String r
twoDigits kont =
  unconsAscii (unexpectedEOF "a digit") $ \c1 ->
    if
      | '0' <= c1
      , c1 <= '9' -> unconsAscii (unexpectedEOF "a digit") $ \c2 ->
          if
...
munchDigits_ ::
  ...
  Int ->
  Either String r
munchDigits_ kontEOF kontC arr = loop
  where
    loop off0 off len = unconsAscii_ arr off len (inline kontEOF (Text arr off0 (off - off0))) $ \c off' len' ->
      if
...
parseYear_ ::
  ...
  Text ->
  Either String r
parseYear_ kontEOF kontC (Text arr offS lenS) = start offS lenS
  where
    start :: Int -> Int -> Either String r
    start !off !len = unconsAscii_
      arr
      off
      len
      (unexpectedEOF "-, +, or a digit")
      $ \c off' len' -> case c of
    ...
    loop !posNeg !off0 !off !len = unconsAscii_ arr off len (finishEOF posNeg off0 off) $ \c off' len' ->
      if
...
parseQuarterOfYear_ kont =
  unconsAscii (unexpectedEOF "QuarterOfYear") $ \c ->
    if
...
parseTimeOfDay__ ::
  ...
  Text ->
  Either String r
parseTimeOfDay__ kontEOF kontC =
  parseTimeOfDay_
